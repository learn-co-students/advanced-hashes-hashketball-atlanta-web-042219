...
def num_points_scored(player_name)
  ...
  points = player_hash.fetch(player_name)
  points.fetch(:points)
end

def shoe_size(player_name)
  player_hash = game_hash[:home][:players].merge(game_hash[:away][:players])
  shoe = player_hash.fetch(player_name)
  shoe.fetch(:shoe)
end
...
def player_numbers(team_name)
  combined = game_hash.values
  teams = combined.find {|team| team.fetch(:team_name) == team_name}
  teams[:players].map {|name, stats| stats[:number]}
end

def player_stats(players_name)
  player_hash = game_hash[:home][:players].merge(game_hash[:away][:players])
  stats = player_hash.fetch(players_name)
end
